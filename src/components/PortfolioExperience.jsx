...
function MacBook({ position }) {
  ...
  useEffect(() => {
    if (!lidRef.current || !screenRef.current) return;
    ...
    // Screen unlock animation (blur to clear effect)
    gsap.fromTo(screenContentRef.current.material,
      { opacity: 0, emissiveIntensity: 0 },
      { 
        opacity: 1, 
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      {/* MacBook Lid */}
      <mesh ref={lidRef} position={[0, 0, 0]} rotation={[0.5, 0, 0]} castShadow>
...
function FileFolder({ position }) {
  ...
  useEffect(() => {
    ...
    // Folder opening animation with elastic effect
    ...
    // Project cards flying out in circular pattern
  ...
  return (
    <group position={position}>
      {/* Folder */}
      <mesh ref={folderRef} rotation={[0, 0, 0]} castShadow>
...
function IPad({ position }) {
  ...
  useEffect(() => {
    ...
    // iPad scaling animation with elastic effect
  ...
  return (
    <group position={position}>
      <mesh ref={ipadRef} scale={[0.8, 0.8, 0.8]} castShadow>
        {/* iPad Body */}
        <mesh>
